...
def takeScreenshot(y1, y2, subject, paperNumber, currentQuestion, i, subjectCode):
    ...
    with Image.open(f"{output1Path}/{subject}/p{paperNumber}/{i + int(initialPage)}.jpg") as im:
            im_crop = im.crop((110, y1, 2310, y2))
...
def dimensions(currentPage, prevPixel, subject, paperNumber): # i'm currently testing this
    im = Image.open(f"{output1Path}/{subject}/p{paperNumber}/{currentPage}.jpg")
    pix = im.load()
    found = False
    i = 1
    while found == False:
        if (pix[147,prevPixel + i] == "(255, 255, 255)") and (pix[147,prevPixel + i + i] != "(255, 255, 255)"):
            found = True
            return ((prevPixel + i) - 50)
        else:
            i += 1
print(isBlankPage(11))